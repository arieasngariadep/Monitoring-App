...
@section('breadcrumbs')
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">List Komplain {{ $judul }}</li>
    </ol>
@endsection('breadcrumbs')

@section('title', 'List Komplain '.$judul.'')
@section('titleTab', 'List Komplain '.$judul.'')
...
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <form action="<?= route('getListKomplain') ?>" method="GET">
                    <div class="row mt-3">
                        <div class="col-md-3">
                        ...
                        <div class="col-md-3">
                            <select name="jenis_komplain" id="jenis_komplain" class="form-control">
                                ...
                                <option value="ATM" <?= $jenis_komplain == 'ATM' ? 'selected' : '' ?>>ATM</option>
                                <option value="EDC" <?= $jenis_komplain == 'EDC' ? 'selected' : '' ?>>EDC</option>
                            </select>
                        </div>
                        <div class="col-md-3">
                            <button type="submit" class="btn btn-primary" style="width: 200px; height: 38px;"><i class="dripicons-search"></i>&nbsp;&nbsp;Cari</button>
                        </div>
                    </div>
                </form>

                @if($bulan1 != NULL || $bulan2 != NULL || $jenis_komplain != NULL)
                <div class="row mt-4">
                    <div class="col-md-5 text-right">
                        <form action="{{ route('exportReportKomplain') }}" method="POST">
                            {{ csrf_field() }}
                            ...
                            <input autocomplete="off" type="text" class="form-control" id="jenis_komplain" placeholder="Jenis Komplain" name="jenis_komplain" value="<?= $jenis_komplain ?>" hidden />
                    ...
                    <div class="col-md-2 text-center">
                        <form action="<?= route('prosesDeleteKomplainBulk') ?>" method="POST">
                            {{ csrf_field() }}
                            {{ method_field('DELETE') }}
                            ...
                            <input type="text" class="form-control" name="jenis_komplain" value="<?= $jenis_komplain ?>" hidden />
                ...
                <table class="table table-bordered table-striped table-md" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    <thead>
                        <tr>
                            ...
                            <th colspan="2" style="text-align:center; vertical-align: middle;">Sesuai SLA</th>
                            <th colspan="2" style="text-align:center; vertical-align: middle;">Tidak Sesuai SLA</th>
                    ...
                    <tbody>
                        @if(count($listData) > 0)
                            @foreach($listData as $list)
                                ...
                                <tr>
                                    <td>{{ $tanggal }}</td>
                                    <td>{{ number_format($list->jumlah_sesuai, 0, '', '.') }}</td>
                                    <td>{{ round($list->persentase_sesuai, 2) }} %</td>
                                    <td>{{ number_format($list->jumlah_tidak_sesuai, 0, '', '.') }}</td>
                                    <td>{{ round($list->persentase_tidak_sesuai, 2) }} %</td>
                                    <td>{{ number_format($list->jumlah_komplain, 0, '', '.') }}</td>
                                    <td class='text-center'>
                                        <a href='<?= route('formUpdateKomplain', ['id' => $list->id]) ?>' class='btn btn-outline-success' data-toggle='tooltip' data-placement='top' title='' data-original-title='Form Update'>
                                            <i class='fa fa-edit'></i>
                                        </a> | 
                                        <a href='<?= route('prosesDeleteKomplain', ['id' => $list->id]) ?>' class='btn btn-outline-danger' data-toggle='tooltip' data-placement='top' title='' data-original-title='Delete'>
