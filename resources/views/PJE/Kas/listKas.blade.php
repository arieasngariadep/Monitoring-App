...
@section('breadcrumbs')
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="#">Kas ATM/CRM</a></li>
        <li class="breadcrumb-item active">List Kas ATM/CRM</li>
    </ol>
@endsection('breadcrumbs')

@section('title', 'List Kas ATM/CRM')
@section('titleTab', 'List Kas ATM/CRM')
...
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <form action="<?= route('getListKas') ?>" method="GET">
                    <div class="row mt-3">
                        <div class="col-md-4">
                ...
                <div class="row mt-4">
                    <div class="col-md-5 text-right">
                        <form action="{{ route('exportReportKas') }}" method="POST">
                            {{ csrf_field() }}
                    ...
                    <div class="col-md-2 text-center">
                        <form action="<?= route('prosesDeleteKasBulk') ?>" method="POST">
                            {{ csrf_field() }}
                            {{ method_field('DELETE') }}
                    ...
                    <tbody>
                        @if(count($listData) > 0)
                            @foreach($listData as $list)
                                ...
                                <tr>
                                    <td>{{ $tanggal }}</td>
                                    <td>{{ number_format($list->pengambilan, 0, '', '.') }}</td>
                                    <td>{{ number_format($list->pengisian, 0, '', '.') }}</td>
                                    <td>{{ number_format($list->sisa, 0, '', '.') }}</td>
                                    <td>{{ round($list->persentase_pengisian, 2) }} %</td>
                                    <td class='text-center'>
                                        <a href='<?= route('formUpdateKas', ['id' => $list->id]) ?>' class='btn btn-outline-success' data-toggle='tooltip' data-placement='top' title='' data-original-title='Form Update'>
                                            <i class='fa fa-edit'></i>
                                        </a> | 
                                        <a href='<?= route('prosesDeleteKas', ['id' => $list->id]) ?>' class='btn btn-outline-danger' data-toggle='tooltip' data-placement='top' title='' data-original-title='Delete'>
