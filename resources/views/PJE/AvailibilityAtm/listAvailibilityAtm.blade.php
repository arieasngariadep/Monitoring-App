...
@section('breadcrumbs')
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="#">Availibility ATM</a></li>
        <li class="breadcrumb-item active">List Availibility ATM</li>
    </ol>
@endsection('breadcrumbs')

@section('title', 'List Availibility ATM')
@section('titleTab', 'List Availibility ATM')
...
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <form action="<?= route('getListAvailibilityAtm') ?>" method="GET">
                    <div class="row mt-3">
                        <div class="col-md-4">
                ...
                <div class="row mt-4">
                    <div class="col-md-5 text-right">
                        <form action="{{ route('exportReportAvailibilityAtm') }}" method="POST">
                            {{ csrf_field() }}
                    ...
                    <div class="col-md-2 text-center">
                        <form action="<?= route('prosesDeleteAvailibilityAtmBulk') ?>" method="POST">
                            {{ csrf_field() }}
                            {{ method_field('DELETE') }}
                    ...
                    <tbody>
                        @if(count($listData) > 0)
                            @foreach($listData as $list)
                                ...
                                <tr>
                                    <td>{{ $tanggal }}</td>
                                    <td>{{ number_format($list->rata_event_problem, 0, '', '.') }}</td>
                                    <td>{{ round($list->durasi_bulan, 2) }} Menit</td>
                                    <td>{{ round($list->durasi_ytd, 2) }} Menit</td>
                                    <td class='text-center'>
                                        <a href='<?= route('formUpdateAvailibilityAtm', ['id' => $list->id]) ?>' class='btn btn-outline-success' data-toggle='tooltip' data-placement='top' title='' data-original-title='Form Update'>
                                            <i class='fa fa-edit'></i>
                                        </a> | 
                                        <a href='<?= route('prosesDeleteAvailibilityAtm', ['id' => $list->id]) ?>' class='btn btn-outline-danger' data-toggle='tooltip' data-placement='top' title='' data-original-title='Delete'>
